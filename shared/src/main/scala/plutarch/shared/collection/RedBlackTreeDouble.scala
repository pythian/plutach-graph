...
/**
 * Scala's implementation of RB-tree, modified to keep nodes in double-linked list (Double -> T)
 */
object RedBlackTreeDouble {

  @inline private def compare(akey: Double, bkey: Double): Int = {
    if (akey < bkey) -1 else if (akey > bkey) 1 else 0
  }

  final class Tree[B](var root: Node[B], var size: Int)

  final class Node[B](var key: Double, var value: B, var red: Boolean, var left: Node[B], var right: Node[B], var parent: Node[B], var prev: Node[B], var next: Node[B])
  ...
  @tailrec private def getNode[B](node: Node[B], key: Double): Node[B] = {
  ...
  def insert[B](tree: Tree[B], key: Double, value: B): Unit = {
    var y: Node[B] = null
    var x = tree.root
    var cmp = 1
  ...
  def delete[B](tree: Tree[B], key: Double): Unit = {
  ...
  def getNode[B](tree: Tree[B], key: Double): Node[B] = getNode(tree.root, key)

  def minNodeAfter[B](tree: Tree[B], key: Double): Node[B] = {
  ...
  def maxNodeBefore[B](tree: Tree[B], key: Double): Node[B] = {
    if (tree.root eq null) null
    else {
      var y: Node[B] = null
      var x = tree.root
  ...
  def maxNodeBeforeOrFirst[B](tree: Tree[B], key: Double): Node[B] = if (tree.root eq null) null else {
    val node = maxNodeBefore(tree, key)
    if (node eq null) minNodeNonNull(tree) else node
  }

  def iterator[B](tree: Tree[B]): Iterator[(Double, B)] = if (tree.root eq null) Iterator.empty else new Iterator[(Double, B)] {
    private var node = minNodeNonNull(tree.root)
    override def hasNext: Boolean = node ne null
    override def next(): (Double, B) = {
      val res = node
      node = node.next
      (res.key, res.value)
    }
  ...
  def getOrElseUpdate[B](tree: Tree[B], key: Double, value: ⇒ B): B = {
    val node = RedBlackTreeDouble.getNode(tree.root, key)
    if (node eq null) {
      val data = value
      RedBlackTreeDouble.insert(tree, key, data)
      data
    } else {
      node.value
    }
